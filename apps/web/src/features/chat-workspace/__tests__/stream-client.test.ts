...
describe('StreamClient', () => {
  ...
  // ─── T028: Subscription tracking ─────────────────────────────────────────

  describe('subscription tracking', () => {
    it('tracks subscriptions via activeSubscriptions', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();

      client.subscribe('conv-1', 5);
      assert.equal(client.activeSubscriptions.size, 1);
      assert.equal(client.activeSubscriptions.get('conv-1'), 5);
    });

    it('tracks subscribe without resume seq as undefined', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();
      ...
      client.subscribe('conv-1');
      assert.ok(client.activeSubscriptions.has('conv-1'));
      assert.equal(client.activeSubscriptions.get('conv-1'), undefined);
    });
    ...
    it('clears all subscriptions on close()', () => {
      ...
      client.subscribe('conv-1', 5);
      client.subscribe('conv-2', 10);
      assert.equal(client.activeSubscriptions.size, 2);

      client.close();
      assert.equal(client.activeSubscriptions.size, 0);
    });

    it('updates tracked resume seq via updateSubscriptionSeq', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();
      ...
      client.updateSubscriptionSeq('conv-1', 12);
      assert.equal(client.activeSubscriptions.get('conv-1'), 12);
    });

    it('updateSubscriptionSeq is a no-op for untracked conversations', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();

      client.updateSubscriptionSeq('conv-unknown', 10);
      assert.equal(client.activeSubscriptions.size, 0);
    });

    it('tracks multiple concurrent subscriptions', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();
      ...
      client.subscribe('conv-1', 5);
      client.subscribe('conv-2', 10);
      client.subscribe('conv-3');

      assert.equal(client.activeSubscriptions.size, 3);
      assert.equal(client.activeSubscriptions.get('conv-1'), 5);
      assert.equal(client.activeSubscriptions.get('conv-2'), 10);
      assert.equal(client.activeSubscriptions.get('conv-3'), undefined);
    });

    it('activeSubscriptions returns a snapshot (not live reference)', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws = getSocket();
      ws.simulateOpen();
      ...
      client.subscribe('conv-1', 5);
      const snap = client.activeSubscriptions;
      client.subscribe('conv-2', 10);
  ...
  // ─── T028: Reconnect method ──────────────────────────────────────────────

  describe('reconnect', () => {
    it('closes old socket and opens a new one', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ws1.simulateOpen();

      client.reconnect(noopCallbacks());
      const ws2 = getSocket();
      assert.notEqual(ws1, ws2);
      assert.ok(ws1.closeCalled);
    });

    it('resubscribes to all tracked conversations on new socket open', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ws1.simulateOpen();
      ...
      // Should have resubscribed to both conversations
      const subs = ws2.sentMessages.filter(
        (m) => (m as { type: string }).type === 'subscribe',
      ) as { type: string; conversationId: string; lastAcknowledgedSeq?: number }[];

      assert.equal(subs.length, 2);
      const convIds = subs.map((s) => s.conversationId).sort();
      assert.deepStrictEqual(convIds, ['conv-1', 'conv-2']);

      const conv1Sub = subs.find((s) => s.conversationId === 'conv-1');
      assert.equal(conv1Sub?.lastAcknowledgedSeq, 5);

      const conv2Sub = subs.find((s) => s.conversationId === 'conv-2');
      assert.equal(conv2Sub?.lastAcknowledgedSeq, 10);
    });

    it('fires onReconnecting callback before opening new socket', () => {
      const events: string[] = [];
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ...
      const ws2 = getSocket();
      // onReconnecting should fire before onOpen
      assert.deepStrictEqual(events, ['reconnecting']);

      ws2.simulateOpen();
      assert.deepStrictEqual(events, ['reconnecting', 'open']);
    });

    it('preserves subscription tracking across reconnect', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ws1.simulateOpen();
    ...
    it('uses updated resume seq for resubscribe after updateSubscriptionSeq', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ws1.simulateOpen();
      ...
      client.subscribe('conv-1', 5);
      client.updateSubscriptionSeq('conv-1', 20);
      ...
      assert.equal(subs[0].lastAcknowledgedSeq, 20);
    });

    it('works when there are no active subscriptions', () => {
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ws1.simulateOpen();
      ...
      // Should have no subscribe messages
      const subs = ws2.sentMessages.filter((m) => (m as { type: string }).type === 'subscribe');
      assert.equal(subs.length, 0);
    });

    it('works when not currently connected (fresh reconnect)', () => {
      const client = createStreamClient(defaultOptions());
      // Never connected — reconnect should still open a new socket
      client.reconnect(noopCallbacks());
      const ws = getSocket();
      assert.ok(ws);
    ...
    it('dispatches messages on new socket after reconnect', () => {
      const events: string[] = [];
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks());
      const ws1 = getSocket();
      ...
      client.reconnect(
        noopCallbacks({
          onSubscribed: (convId, seq) => events.push(`subscribed:${convId}:${seq}`),
        }),
      );
      const ws2 = getSocket();
      ws2.simulateOpen();

      ws2.simulateMessage({ type: 'subscribed', conversationId: 'conv-1', currentSeq: 42 });
      assert.deepStrictEqual(events, ['subscribed:conv-1:42']);
    });

    it('does not fire callbacks on old socket after reconnect', () => {
      const events: string[] = [];
      const client = createStreamClient(defaultOptions());
      client.connect(noopCallbacks({ onOpen: () => events.push('open-1') }));
      const ws1 = getSocket();
      ws1.simulateOpen();
      events.length = 0;

      client.reconnect(noopCallbacks({ onOpen: () => events.push('open-2') }));
      const ws2 = getSocket();

      // Old socket fires open again (stale) — should be ignored
      ws1.readyState = FakeWebSocket.OPEN;
      ws1.onopen?.({ type: 'open' });
      assert.deepStrictEqual(events, []);

      ws2.simulateOpen();
      assert.deepStrictEqual(events, ['open-2']);
    });
  });
});
