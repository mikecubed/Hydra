...
describe('transcript reconciliation after turn actions', () => {
  ...
  it('terminal authoritative entry does not preserve stale streamed controls', () => {
    const staleControl: EntryControlState = {
      controlId: 'ctl-cancel',
      kind: 'cancel',
      enabled: true,
      reasonDisabled: null,
    };
    ...
    // Stream has an in-flight turn with an actionable cancel control
    state = reduceWorkspaceState(state, {
      type: 'conversation/replace-entries',
      conversationId: 'conv-1',
      entries: [
        createEntry({
          ...
          turnId: 'turn-1',
          status: 'streaming',
          controls: [staleControl],
        }),
      ],
      hasMoreHistory: false,
    });

    // REST authoritative response arrives: turn is cancelled, controls cleared
    state = reduceWorkspaceState(state, {
      type: 'conversation/merge-history',
      conversationId: 'conv-1',
      entries: [
    ...
    assert.equal(conv?.entries[0]?.status, 'cancelled');
    assert.deepEqual(
      conv?.entries[0]?.controls,
      [],
      'terminal authoritative turn must not retain stale streamed controls',
    );
  });

  it('terminal authoritative entry does not preserve stale controls for failed status', () => {
    const staleRetry: EntryControlState = {
      controlId: 'ctl-retry',
      kind: 'retry',
      enabled: true,
      reasonDisabled: null,
    ...
    // Stream has a turn with a retry control from an earlier reconciliation
    state = reduceWorkspaceState(state, {
      type: 'conversation/replace-entries',
      conversationId: 'conv-1',
      entries: [
    ...
    // A second REST merge now says the turn completed (e.g., retry succeeded
    // server-side and original turn was re-resolved). Controls should be empty.
    state = reduceWorkspaceState(state, {
      type: 'conversation/merge-history',
      conversationId: 'conv-1',
      entries: [
    ...
    const conv = state.conversations.get('conv-1');
    assert.equal(conv?.entries[0]?.status, 'completed');
    assert.deepEqual(
      conv?.entries[0]?.controls,
      [],
      'completed authoritative turn must not retain stale retry control',
    );
  });

  it('non-terminal authoritative entry still preserves richer streamed controls', () => {
    const liveControl: EntryControlState = {
      controlId: 'ctl-cancel',
      kind: 'cancel',
      enabled: true,
      reasonDisabled: null,
    ...
    // Stream has a turn with a live cancel control
    state = reduceWorkspaceState(state, {
      type: 'conversation/replace-entries',
      conversationId: 'conv-1',
      entries: [
    ...
    // REST snapshot is behind: still submitted, no controls yet
    state = reduceWorkspaceState(state, {
      type: 'conversation/merge-history',
      conversationId: 'conv-1',
      entries: [
    ...
    const conv = state.conversations.get('conv-1');
    // Non-terminal: stream controls should be preserved
    assert.deepEqual(conv?.entries[0]?.controls, [liveControl]);
  });
});
