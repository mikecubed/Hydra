...
async function handleWorkflowLaunch(
  c: Context<GatewayEnv>,
  daemonClient: DaemonMutationsClient,
  auditService: AuditService | undefined,
): Promise<Response> {
...
function createWorkflowAuditRouter(
  ...
  const app = new Hono<GatewayEnv>();
  app.post('/workflows/launch', (c) => handleWorkflowLaunch(c, daemonClient, auditService));
  app.get('/audit', (c) => handleAuditGet(c, daemonClient));
  return app;
}
