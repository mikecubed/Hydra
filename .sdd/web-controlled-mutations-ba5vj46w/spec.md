...
This spec extends the established mutation pattern from `web-hydra-operations-panels` to the config
domain. The operations-control mutation pattern (CSRF double-submit, POST-based, gateway-proxied,
daemon-authoritative, optimistic concurrency via revision token) is already in production; this spec
applies that same pattern to routing mode, model tier, budget limits, workflow launch, and audit
trail. Every write originates in the browser, passes through the authenticated gateway (which
enforces CSRF validation via `createProtectedRouteGroup`, rate limiting, and session binding), is
...
**Independent Test**: Submit a `POST /config/routing/mode` with valid CSRF headers and a known
...
1. **Given** the current routing mode is `balanced` and the operator selects `economy`,
   **When** the confirm dialog is submitted, **Then** the gateway forwards a CSRF-verified
   `POST /config/routing/mode` to the daemon, the daemon persists the change, and the panel
...
An operator selects a model tier (`default`|`fast`|`cheap`) for any agent that has a `models`
config entry. The four built-in physical agents are `claude`, `gemini`, `codex`, and `local`; the
optional `copilot` agent and any user-defined agents from `agents.customAgents[]` are also
recognized. Agents without a `models` config entry (e.g., `local`, `copilot`, custom `cli`-type
agents) are excluded from model-tier selection and shown as read-only in the config panel. The
operator selects a new tier via a dropdown in the config panel. A confirm dialog shows the agent
name, the current tier, and the new tier before submission.
...
change for `claude` and verify the `POST /config/models/claude/active` request is formed with
the correct body and CSRF headers.

**Acceptance Scenarios**:

1. **Given** `claude` is on `default` tier, **When** the operator selects `fast` and confirms,
   **Then** `POST /config/models/claude/active` is sent with `{"tier":"fast","expectedRevision":"<token>"}`,
...
`POST /config/usage/budget` receives the correct model key and new limit value; verify the
...
1. **Given** the daily budget for `claude-opus-4-6` is `5_000_000`, **When** the operator
   sets it to `3_000_000` and confirms, **Then** `POST /config/usage/budget` is sent, the daemon
...
- **FR-009a**: Workflow launch requests MUST carry a client-generated `idempotencyKey` (UUID) in
  the request body. The daemon MUST deduplicate launches with the same key within a 60-second
  window and return the existing task ID rather than launching a second instance.
...
- **FR-016**: The system MUST maintain a dual-log audit design:
  - **Daemon audit log** (`mutation-audit.jsonl`): durable record of every mutation that reached
    the daemon, with before/after values. Owned by the daemon, served via `GET /audit`.
  - **Gateway attempt log**: lightweight record of every gateway-level rejection (CSRF fail,
    session fail, validation fail, rate-limit) appended to the existing `AuditStore`. Does not
    include before/after values.
  - The browser `GET /audit` endpoint returns only the daemon audit log (durable mutations).
    Gateway rejections are operational logs, not shown in the audit trail panel.
...
This module follows the exact same gateway proxy pattern established by
`apps/web-gateway/src/operations/` — same auth middleware stack, same CSRF wiring via
`createProtectedRouteGroup`, same `GatewayErrorResponse` five-category error model. Routes are
registered in `apps/web-gateway/src/index.ts` via `createProtectedRouteGroup()` and
`createProtectedRootRoutes()`, matching the pattern used by the `operations` module.

All routes require:

- Valid authenticated session (`__session` cookie — cookie-only, no `Authorization` header)
- CSRF double-submit validation is applied globally by `createProtectedRouteGroup()` for all
  non-GET methods (POST); individual routes do not wire CSRF themselves
- JSON `Content-Type` on request bodies

| Method | Path                           | Description                                  | Auth | CSRF |
| ------ | ------------------------------ | -------------------------------------------- | ---- | ---- |
| `GET`  | `/config/safe`                 | Returns `SafeConfigView`                     | ✓    | —    |
| `POST` | `/config/routing/mode`         | Mutates global routing mode                  | ✓    | ✓    |
| `POST` | `/config/models/:agent/active` | Mutates active model tier for `:agent`       | ✓    | ✓    |
| `POST` | `/config/usage/budget`         | Mutates daily/weekly budget limits           | ✓    | ✓    |
...
- **Path param** `:agent`: any agent registered in `agents.customAgents[]` plus the four built-in
  physical agents (`claude`, `gemini`, `codex`, `local`) and optional `copilot`. Agents without a
  `models` config entry (e.g., `local`, `copilot`, custom `cli`-type agents) are excluded from
  model-tier selection and return 400. Validated at the gateway; 400 on ineligible agent.
- **Request body**: `{ "tier": "default"|"fast"|"cheap", "expectedRevision": string }`
- **Response 200**: `ConfigMutationResponse`
- **Response 400**: Invalid tier or ineligible/unknown agent
- **Response 403**: CSRF validation failure
...
- **Request body**: `{ "workflow": "evolve"|"tasks"|"nightly", "label": string | null, "idempotencyKey": string (UUID), "expectedRevision": string }`
- **Response 202**: `WorkflowLaunchResponse`
- **Response 400**: Unknown workflow name or missing idempotency key
- **Response 403**: CSRF failure
...
### 4.2 Daemon Endpoints (new, in `lib/daemon/mutation-routes.ts`)

| Method | Path                           | Description                                    |
| ------ | ------------------------------ | ---------------------------------------------- |
| `GET`  | `/config/safe`                 | Returns safe config subset with revision token |
| `POST` | `/config/routing/mode`         | Persists routing mode change                   |
| `POST` | `/config/models/:agent/active` | Persists agent model tier change               |
| `POST` | `/config/usage/budget`         | Persists budget limit change                   |
...
- Generating and validating `revision` tokens (content hash of the mutable config sections)
- Persisting all mutations to `hydra.config.json`
- Assigning task IDs for workflow launches
- Deduplicating workflow launches via `idempotencyKey` within a 60-second window (FR-009a)
- Appending `MutationAuditRecord` entries to the **daemon audit log** (`mutation-audit.jsonl`) on
  every mutation that reaches the daemon, with before/after values (durable record)
- Serving the daemon audit log via `GET /audit` (the browser audit trail panel reads only this log;
  gateway-level rejections are recorded in the gateway's own `AuditStore` and are not surfaced here)
...
- `AgentId`: dynamically derived from the four built-in physical agents (`claude`, `gemini`,
  `codex`, `local`) plus optional `copilot`, plus any user-defined agents from
  `agents.customAgents[]`. Agents without a `models` config entry (e.g., `local`, `copilot`,
  custom `cli`-type agents) are excluded from model-tier selection and shown as read-only.
...
- `WorkflowLaunchRequest` Zod schema (includes `idempotencyKey: z.string().uuid()`)
...
- **SEC-01 — CSRF**: All `POST` mutation routes at the gateway MUST validate the
  double-submit CSRF token before forwarding any request body to the daemon. CSRF is applied
  globally by `createProtectedRouteGroup()` in `apps/web-gateway/src/index.ts`, not per-route.
  The existing CSRF middleware in `apps/web-gateway/src/security/` MUST be applied to the
  mutations router via this function.
...
- **SEC-04 — Output sanitization**: `SafeConfigView` MUST be validated against a Zod schema
  using `.strip()` before being returned to the browser. Any fields not explicitly included
  in the schema MUST be silently stripped by the schema parse (not rejected). A `.superRefine()`
  guard MUST reject any key whose name matches `/(apiKey|secret|hash|password)/i`.
...
- **SEC-06 — Audit completeness**: Every mutation that reaches the daemon MUST produce a
  `MutationAuditRecord` entry in the daemon audit log (`mutation-audit.jsonl`) with before/after
  values. Every gateway-level rejection (CSRF fail, session fail, validation fail, rate-limit)
  MUST be recorded in the gateway's existing `AuditStore` before the response is sent. A failed
  audit write MUST NOT silently suppress the mutation; it MUST be logged as an error in the
...
The `mutations-routes.ts` router is registered in `apps/web-gateway/src/index.ts` via
`createProtectedRouteGroup()`, matching the same pattern used by the `operations` module.
`GATEWAY_ROUTE_PREFIXES` in `apps/web-gateway/src/server-runtime.ts` must be updated to include
`/mutations` and `/audit` so that GET requests to these paths are not intercepted by the SPA
...
- Gateway `GET /audit` cursor pagination: verify `nextCursor` forwarded correctly and response
  schema validated
- Optimistic concurrency race: two concurrent `POST /config/routing/mode` requests with the
...
- **Deferred budget fields**: The following budget-related config fields are deferred from the
  mutable surface: `warningThresholdPercent`, `criticalThresholdPercent`, `windowTokenBudget`,
  `sessionBudget`, `perTaskBudget`, `perAgentBudget`. In-scope budget mutations are limited to:
  `dailyTokenBudget` per-agent and `weeklyTokenBudget` per-agent only.
...
- **SC-007**: `SafeConfigView` uses `.strip()` and passes a test asserting that extra fields are
  silently stripped on parse (not rejected). The `.superRefine()` guard rejects forbidden key names.
