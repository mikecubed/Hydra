...
describe('hydra daemon state-machine characterization', () => {
  it('moves tasks through claim, update, result completion, and auto-unblock flow', { timeout: 60_000 }, async (t) => {
    ...
    const parent = await requestJson<{ task: { id: string } }>(daemon.baseUrl, 'POST', '/task/add', {
  ...
  it('serializes concurrent claims and records worker heartbeats', { timeout: 60_000 }, async (t) => {
    const projectRoot = createTempProject({
    ...
    const stale = await requestJson<{ tasks: Array<{ id: string }> }>(daemon.baseUrl, 'GET', '/tasks/stale');
    assert.equal(stale.response.status, 200);
    assert.deepEqual(stale.json.tasks, []);
  });

  it('rejects missing fields, invalid task IDs, invalid statuses, and circular dependencies', { timeout: 60_000 }, async (t) => {
    const projectRoot = createTempProject({
    ...
    const missingTaskId = await requestJson<{ error?: string }>(daemon.baseUrl, 'POST', '/task/update', {
      notes: 'Missing task id',
    });
    assert.equal(missingTaskId.response.status, 400);
    assert.match(missingTaskId.json.error ?? '', /taskId/i);

    const missingAgent = await requestJson<{ error?: string }>(daemon.baseUrl, 'POST', '/task/result', {
      taskId: 'T404',
    });
    assert.equal(missingAgent.response.status, 400);
    assert.match(missingAgent.json.error ?? '', /taskId.*agent|agent.*taskId/i);

    const first = await requestJson<{ task: { id: string } }>(daemon.baseUrl, 'POST', '/task/add', {
      title: 'First dependency node',
    });
    const second = await requestJson<{ task: { id: string } }>(daemon.baseUrl, 'POST', '/task/add', {
      title: 'Second dependency node',
    });

    const invalidTask = await requestJson<{ error?: string }>(daemon.baseUrl, 'POST', '/task/update', {
      taskId: 'T999',
      status: 'done',
    });
    ...
    const circular = await requestJson<{ error?: string }>(daemon.baseUrl, 'POST', '/task/update', {
    ...
    const invalidStatus = await requestJson<{ error?: string }>(daemon.baseUrl, 'POST', '/task/update', {
      taskId: first.json.task.id,
      status: 'failed',
    });
  ...
  it('dead-letters repeated worker errors and allows retrying the task', { timeout: 60_000 }, async (t) => {
    const projectRoot = createTempProject({
    ...
    const claim = await requestJson<{ task: { id: string } }>(daemon.baseUrl, 'POST', '/task/claim', {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      const reopen: RequestResult<{ task: { status: string; owner: string } }> = await requestJson(
        daemon.baseUrl,
        'POST',
        '/task/update',
        {
