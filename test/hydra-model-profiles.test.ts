...
describe('ROLE_DEFAULTS', () => {
  it('has all expected roles', () => {
    ...
    for (const role of expected) {
      assert.ok((ROLE_DEFAULTS as Record<string, unknown>)[role], `Missing role: ${role}`);
    }
  });
...
describe('AGENT_PRESETS', () => {
  it('has presets for all 3 agents', () => {
    for (const agent of ['claude', 'codex', 'gemini']) {
      const presets = (AGENT_PRESETS as Record<string, Record<string, string>>)[agent];
...
describe('query functions', () => {
  ...
  it('getProfile returns null for unknown model', () => {
    ...
    assert.equal(getProfile(null as unknown as string), null);
  });
