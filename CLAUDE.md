...
- **`hydra-model-recovery.mjs`** — Post-hoc model error detection and fallback. When an agent CLI fails because the configured model is unavailable, detects the error and offers fallback selection. Exports `detectModelError(agent, result)`, `getFallbackCandidates(agent, failedModel)`, `recoverFromModelError(agent, failedModel, opts)`, `isModelRecoveryEnabled()`. Interactive mode uses `promptChoice()` for user selection; headless mode auto-selects first candidate. Config: `modelRecovery.enabled`, `.autoPersist`, `.headlessFallback`.
