...
import { detectModelError, detectCodexError, recoverFromModelError, detectRateLimitError, detectUsageLimitError, formatResetTime, calculateBackoff } from './hydra-model-recovery.mjs';
...
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  ...
  if (usageCheck.isUsageLimit) {
    ...
    notifyDoctor(doctorPayload(agent, result, opts, { error: usageCheck.errorMessage, context: `Usage limit — resets in ${resetLabel}` }));
