...
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  ...
  if (usageCheck.isUsageLimit) {
    ...
    if (verification.verified === false) {
      // API returned 200 — account is active. Pattern match was a false positive.
      // Log and fall through to rate-limit handling.
      log.dim(`${label}: usage limit pattern matched but API says account is active (false positive) — pattern: "${usageCheck.errorMessage.slice(0, 80)}"`);
      ...
      const verifyNote = confirmed ? 'confirmed by API' : `unverified (${verification.reason || 'no API key'})`;
      log.warn(`${label}: usage limit reached — ${verifyNote} — resets in ${resetLabel}`);
      log.dim(`  Triggered by: "${usageCheck.errorMessage.slice(0, 120)}"`);
      disabledAgents.add(agent);
      notifyDoctor(doctorPayload(agent, result, opts, { error: usageCheck.errorMessage, context: `Usage limit ${verifyNote} — resets in ${resetLabel}` }));
      result.usageLimited = true;
      result.usageLimitConfirmed = confirmed;
