...
function executeAgent(agent, prompt, { cwd, timeoutMs, modelOverride, phaseLabel } = {}) {
  return new Promise((resolve) => {
    ...
    // Gemini CLI v0.27.x has a stdin duplication bug: piped messages are sent
    // twice, causing 'candidates' undefined crashes. Use -p arg for Gemini instead.
    // Claude and Codex use stdin to avoid Windows cmd.exe length/escaping issues.
    let useStdin;
