...
export async function scanStaleTaskWorktrees(projectRoot) {
  ...
  } catch { /* fs or config unavailable */ }
  return items;
}
...
async function executeWorktreeCleanup(item, projectRoot, startMs) {
  ...
  try {
    const { git } = await import('./hydra-shared/git-ops.mjs');
    ...
    return { item, ok: true, output: `Removed stale worktree ${path.basename(worktreePath)}`, durationMs: Date.now() - startMs };
  } catch (err) {
    return { item, ok: false, error: err.message, durationMs: Date.now() - startMs };
  }
}
