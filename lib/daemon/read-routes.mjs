...
export async function handleReadRoute(ctx) {
  ...
  if (method === 'GET' && route === '/activity') {
    ...
    for (const name of ['claude', 'gemini', 'codex']) {
      ...
      const currentTask =
        state.tasks.find((t) => t.owner === name && t.status === 'in_progress') || null;
    ...
    const todo = state.tasks
      ...
      .map((t) => ({ id: t.id, title: t.title, owner: t.owner, type: t.type }));
    const blocked = state.tasks
      .filter(
        (t) =>
          t.status === 'blocked' ||
          (Array.isArray(t.blockedBy) && t.blockedBy.some((dep) => !completedIds.has(dep))),
      )
      .map((t) => ({ id: t.id, title: t.title, owner: t.owner, blockedBy: t.blockedBy }));
    const recentlyCompleted = state.tasks
      .filter((t) => t.status === 'done')
      .slice(-5)
      .map((t) => ({ id: t.id, title: t.title, owner: t.owner, updatedAt: t.updatedAt }));
  ...
  if (method === 'GET' && route === '/worktrees') {
    try {
      const { listWorktrees, isWorktreeEnabled } = await import('../hydra-worktree.ts');
