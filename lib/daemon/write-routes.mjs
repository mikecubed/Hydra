...
export async function handleWriteRoute(ctx) {
  ...
  if (method === 'POST' && route === '/task/claim') {
    ...
    const wantsWorktree = isNewTask && typeof createTaskWorktree === 'function' && (
    ...
    if (wantsWorktree) {
      ...
      } catch { /* worktree creation is non-critical */ }
    }
  ...
  if (method === 'POST' && route === '/task/result') {
    ...
    const taskDone = ['done'].includes(completedTask?.status);
    if (taskDone && completedTask?.worktreePath && typeof mergeTaskWorktree === 'function') {
      try {
        ...
        if (mergeResult.ok) {
          ...
          await enqueueMutation(`task:worktree_conflict id=${taskId}`, (state) => {
            const t = state.tasks.find((x) => x.id === taskId);
            if (t) {
              t.worktreeConflict = true;
              t.updatedAt = nowIso();
            }
          }, { event: 'worktree_conflict', taskId });
        }
        // On other errors: logged by mergeTaskWorktree, proceed without cleanup
      } catch { /* non-critical — merge failure must not fail the result POST */ }
    }
