...
import { git, getCurrentBranch } from './hydra-shared/git-ops.mjs';
...
export async function createWorktree(taskId, projectRoot, baseBranch = null) {
  ...
  let base = baseBranch || getCurrentBranch(projectRoot) || 'HEAD';
  ...
  if (r.status !== 0) {
    ...
    if (r2.status !== 0) {
      throw new Error(`Failed to create worktree for ${taskId}: ${r2.stderr || r.stderr || 'unknown error'}`);
    }
  }
...
export async function mergeWorktree(taskId, projectRoot, targetBranch = null) {
  ...
  return { ok: false, message: `Merge failed: ${(r.stderr || r.stdout || 'unknown error').trim()}` };
}
