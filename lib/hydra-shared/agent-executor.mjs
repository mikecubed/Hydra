...
import { extractCodexText, extractCodexUsage, extractCodexErrors as _extractCodexErrors } from './codex-helpers.mjs';
...
export function diagnoseAgentError(agent, result) {
  ...
  if (getAgent(agent)?.features?.jsonOutput) {
    ...
    if (jsonlErrors.length > 0) {
      ...
      result.errorDetail = `${agent} reported ${jsonlErrors.length} error(s): ${jsonlErrors.join('; ').slice(0, 200)}`;
...
async function executeCustomCliAgent(agentName, prompt, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', async (code, signal) => {
      ...
      let parsedOutput, tokenUsage = null, costUsd = null;
      ...
      recordCallComplete(metricsHandle, { output: parsedOutput, stdout: parsedOutput, tokenUsage, costUsd });
      await endAgentSpan(span, { ok: !timedOut });
      resolve({
        ok: !timedOut, output: parsedOutput, stdout: parsedOutput, stderr,
        tokenUsage, costUsd,
...
export async function executeAgent(agent, prompt, opts = {}) {
  ...
  // Note: hydra-agents.mjs and agent-executor.mjs have a direct import cycle
  // (agents → codex-helpers; executor → agents); this sentinel avoids adding
  // an extra runtime dependency on Gemini's invoke path.
  // When the Gemini CLI bug is fixed, delete this block and the sentinel check below.
  const headlessInvoke = agentDef.name === 'gemini'
  ...
  if (!headlessInvoke) {
    throw new Error(`Agent "${agent}" has no headless invoke method`);
  }
  ...
  if (_headlessCmd === '__gemini_direct__') {
    ...
    try { return await executeGeminiDirect(_headlessArgs.prompt, geminiOpts); } finally { _hubCleanup(); }
  }
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
      ...
      let costUsd = null;
      {
        ...
        if (_agentDef) {
          try {
            const parsed = _agentDef.parseOutput(rawOutput, { jsonOutput: _agentDef.features.jsonOutput });
        ...
        if (_agentDef?.features?.jsonOutput) {
          try { jsonlErrors = extractCodexErrors(rawOutput); } catch { /* ignore */ }
        }
      }
