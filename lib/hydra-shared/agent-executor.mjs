...
export function executeAgent(agent, prompt, opts = {}) {
  ...
  // OTel tracing: start span, end on resolve
  const spanPromise = startAgentSpan(agent, modelOverride || getActiveModel(agent) || 'unknown', {
    phase: phaseLabel,
  ...
  return new Promise((resolve) => {
    ...
    if (agent === 'codex') {
      ...
      // claude
      const claudePerm = permissionMode || 'auto-edit';
