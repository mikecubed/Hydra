  ...
  try {
    ...
    if (usageCheck.isUsageLimit) {
      ...
      if (verification.verified === false) {
        ...
        // Fall through to rate-limit handling so the caller gets a retry.
        ...
        result.usageLimitConfirmed = verification.verified === true;
        result.resetInSeconds = usageCheck.resetInSeconds;
        result.usageLimitDetail = usageCheck.errorMessage;
        result.error = result.usageLimitConfirmed
          ? `${agent} usage limit confirmed by API (resets in ${resetLabel})`
          : `${agent} usage limit detected — pattern: "${usageCheck.errorMessage.slice(0, 80)}" — resets in ${resetLabel} (unverified: ${verification.reason || 'no API key'})`;
        finalResult = result;
        return result;
      }
    }
