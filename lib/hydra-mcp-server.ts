...
import { z } from 'zod';
import { parseArgs, request } from './hydra-utils.ts';
import { executeAgent, executeAgentWithRecovery } from './hydra-shared/agent-executor.ts';
import { forgeAgent, listForgedAgents } from './hydra-agent-forge.ts';
...
server.registerTool(
  'hydra_ask',
  {
    ...
    inputSchema: {
      agent: z
        .enum(['gemini', 'codex'])
        .describe('Which agent to ask: "gemini" (analyst, reviewer) or "codex" (implementer)'),
      prompt: z.string().min(1).describe('The prompt to send to the agent'),
      system: z
        .string()
        .optional()
        .describe('Optional system instruction to prepend to the prompt'),
  ...
  'hydra_tasks_update',
  {
    ...
    inputSchema: {
      ...
      claimToken: z
        .string()
        .optional()
        .describe('Claim token for atomic update (from hydra_tasks_claim)'),
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
  ...
  'hydra_forge',
  {
    ...
    inputSchema: {
      description: z
        .string()
        .min(1)
        .describe('What the agent should specialize in (e.g. "API testing specialist")'),
      name: z.string().optional().describe('Optional lowercase-hyphenated name override'),
      baseAgent: z
        .enum(['claude', 'gemini', 'codex'])
        .optional()
        .describe('Optional base agent override'),
    ...
    contents: [
      {
        ...
        mimeType: 'application/json',
        text: JSON.stringify(
          buildSelfSnapshot({ includeAgents: false, includeConfig: true, includeMetrics: true }),
          null,
          2,
        ),
      },
...
server.registerPrompt(
  ...
  {
    description:
      'Council deliberation template with role assignments for multi-agent structured reasoning',
...
server.registerPrompt(
  ...
  {
    description:
      'Code review prompt optimized for multi-agent review across security, performance, and correctness',
    argsSchema: {
