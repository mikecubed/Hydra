...
export function getHealthiestProvider(candidates) {
  ...
  return [...candidates].sort((a, b) => {
    ...
    // Score: average of available percentages (higher = healthier)
    const scoreA = computeHealthScore(capA);
    const scoreB = computeHealthScore(capB);

    return scoreB - scoreA; // descending (healthiest first)
  });
}

function computeHealthScore(cap) {
  const pcts = [cap.pctRpm, cap.pctTpm, cap.pctRpd].filter(v => v != null);
  if (pcts.length === 0) return 50; // no data → neutral score
