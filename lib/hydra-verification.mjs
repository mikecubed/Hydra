...
/**
 * Allowlist of safe verification command prefixes.  Verification commands are
 * executed via the system shell (exec/spawn with shell:true), so an attacker
 * that can write hydra.config.json could otherwise inject arbitrary commands.
 * The allowlist prevents obviously dangerous patterns while still supporting
 * every legitimate build/test tool.
 *
  ...
  if (rawCommand && lowered !== 'auto') {
    if (!isVerificationCommandSafe(rawCommand)) {
      return {
        ...
        command: '',
        source: 'config',
        reason: `verification.command rejected: contains unsafe characters (${rawCommand})`,
      };
    }
    return {
      enabled: true,
