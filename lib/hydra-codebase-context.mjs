...
- callAgent(agent, prompt, timeoutMs) -> { ok, stdout, stderr } [hydra-council.mjs]
- extractTasksFromOutput(entries) [hydra-council.mjs]
- extractRisks(entries) [hydra-council.mjs]
...
export function getBaselineContext() {
  ...
  lines.push('5 dispatch modes: auto (classify+route), council (4-phase deliberation), dispatch (pipeline), smart (tier-select), chat (concierge)');
  ...
  const keyModules = [
    ...
    'hydra-council.mjs - 4-phase deliberation: propose->critique->refine->implement',
  ...
  lines.push('models, aliases, modeTiers, usage, verification, concierge, roles, recommendations, agents, evolve, github, workers');
