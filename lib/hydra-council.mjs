...
export function extractAssumptionAttacks(parsed) {
  ...
  for (const bucket of buckets) {
    ...
    for (const item of bucket) {
      ...
      const challenge = cleanText(item.challenge || item.critique || item.text);
      const assumption = cleanText(item.assumption || item.target);
...
async function runAdversarialCouncil(prompt, report, { preview, timeoutMs, specContent, promptHash }) {
  const DIVERGE_AGENTS = ['claude', 'gemini', 'codex'];
  const ATTACK_AGENTS = ['claude', 'gemini', 'codex'];

  // ── Phase 1: DIVERGE (parallel, no shared context) ──
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'start', phase: 'diverge', agents: DIVERGE_AGENTS }) + '\n');
  const divergeSpinner = createSpinner(
    `${DIM('diverge')} ${DIVERGE_AGENTS.map(colorAgent).join(' ')} ${DIM('(parallel)')}`,
    { style: 'orbital' }
  );
  divergeSpinner.start();
  const divergeEntries = [];

  if (preview) {
    for (const agent of DIVERGE_AGENTS) {
      const entry = { round: 1, agent, phase: 'diverge', ok: true, rawText: '{}', parsed: { view: `${agent} diverge preview` }, error: '' };
      divergeEntries.push(entry);
      report.transcript.push(entry);
    }
    divergeSpinner.succeed(`${DIM('diverge')} complete (preview)`);
  } else {
    const divergeStart = Date.now();
    const divergeResults = await Promise.allSettled(
      DIVERGE_AGENTS.map(async (agent) => {
        const p = buildDivergePrompt(agent, prompt, specContent);
        const result = await callAgentAsync(agent, p, timeoutMs);
        return { agent, result };
      })
    );
    divergeSpinner.succeed(`${DIM('diverge')} complete ${DIM(`(${formatElapsed(Date.now() - divergeStart)})`)}`);
    for (const settled of divergeResults) {
      if (settled.status === 'rejected') continue;
      const { agent, result } = settled.value;
      const parsed = parseJsonLoose(result.stdout);
      const entry = { round: 1, agent, phase: 'diverge', ok: result.ok, rawText: result.stdout, parsed, error: result.error || result.stderr || '', recovered: result.recovered || false, recoveredFrom: result.originalModel, recoveredTo: result.newModel };
      divergeEntries.push(entry);
      report.transcript.push(entry);
    }
    saveCheckpoint(promptHash, prompt, 1, 0, report.transcript, specContent);
  }
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'complete', phase: 'diverge' }) + '\n');

  // ── Phase 2: ATTACK (parallel, each sees all diverge outputs) ──
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'start', phase: 'attack', agents: ATTACK_AGENTS }) + '\n');
  const attackSpinner = createSpinner(
    `${DIM('attack')} ${ATTACK_AGENTS.map(colorAgent).join(' ')} ${DIM('(parallel)')}`,
    { style: 'orbital' }
  );
  attackSpinner.start();
  const attackEntries = [];
  ...
  if (preview) {
    for (const agent of ATTACK_AGENTS) {
      const entry = { round: 1, agent, phase: 'attack', ok: true, rawText: '{}', parsed: { assumption_attacks: [] }, error: '' };
      attackEntries.push(entry);
      report.transcript.push(entry);
    }
    attackSpinner.succeed(`${DIM('attack')} complete (preview)`);
  } else {
    const attackStart = Date.now();
    const attackResults = await Promise.allSettled(
      ATTACK_AGENTS.map(async (agent) => {
        const p = buildAttackPrompt(agent, prompt, divergeEntries, specContent);
        const result = await callAgentAsync(agent, p, timeoutMs);
        return { agent, result };
      })
    );
    attackSpinner.succeed(`${DIM('attack')} complete ${DIM(`(${formatElapsed(Date.now() - attackStart)})`)}`);
    for (const settled of attackResults) {
      if (settled.status === 'rejected') continue;
      const { agent, result } = settled.value;
      const parsed = parseJsonLoose(result.stdout);
      const entry = { round: 1, agent, phase: 'attack', ok: result.ok, rawText: result.stdout, parsed, error: result.error || result.stderr || '', recovered: result.recovered || false, recoveredFrom: result.originalModel, recoveredTo: result.newModel };
      attackEntries.push(entry);
      report.transcript.push(entry);
    }
    saveCheckpoint(promptHash, prompt, 1, 1, report.transcript, specContent);
  }
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'complete', phase: 'attack' }) + '\n');

  // ── Phase 3: SYNTHESIZE (Claude as designated decider) ──
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'start', phase: 'synthesize', agent: 'claude' }) + '\n');
  const synthesizeSpinner = createSpinner(`${colorAgent('claude')} ${DIM('synthesize')}`, { style: 'orbital' });
  synthesizeSpinner.start();

  if (preview) {
    report.transcript.push({ round: 1, agent: 'claude', phase: 'synthesize', ok: true, rawText: '{}', parsed: { view: 'synthesize preview', decision: { summary: 'preview', confidence: 'high', next_action: 'handoff', reversible_first_step: 'preview step', tradeoffs: {} } }, error: '' });
    synthesizeSpinner.succeed(`${colorAgent('claude')} ${DIM('synthesize')} complete (preview)`);
  } else {
    const synthesizeStart = Date.now();
    const synthesizeResult = await callAgentAsync('claude', buildSynthesizePrompt(prompt, divergeEntries, attackEntries, specContent), timeoutMs);
    synthesizeSpinner.succeed(`${colorAgent('claude')} ${DIM('synthesize')} complete ${DIM(`(${formatElapsed(Date.now() - synthesizeStart)})`)}`);
    report.transcript.push({ round: 1, agent: 'claude', phase: 'synthesize', ok: synthesizeResult.ok, rawText: synthesizeResult.stdout, parsed: parseJsonLoose(synthesizeResult.stdout), error: synthesizeResult.error || synthesizeResult.stderr || '', recovered: synthesizeResult.recovered || false, recoveredFrom: synthesizeResult.originalModel, recoveredTo: synthesizeResult.newModel });
    saveCheckpoint(promptHash, prompt, 1, 2, report.transcript, specContent);
  }
  process.stderr.write(JSON.stringify({ type: 'council_phase', action: 'complete', phase: 'synthesize', agent: 'claude' }) + '\n');

  // ── Phase 4: IMPLEMENT (Codex, same as sequential — receives full transcript including synthesis) ──
  ...
  if (preview) {
    report.transcript.push({ round: 1, agent: 'codex', phase: 'implement', ok: true, rawText: '{}', parsed: { consensus: 'implement preview', task_allocations: defaultTasks(prompt).map((t) => ({ owner: t.owner, title: t.title, rationale: t.rationale, definition_of_done: t.done })) }, error: '' });
    implementSpinner.succeed(`${colorAgent('codex')} ${DIM('implement')} complete (preview)`);
  } else {
...
async function main() {
  ...
  if (councilMode === 'adversarial') {
    report.councilFlow = ['claude:diverge', 'gemini:diverge', 'codex:diverge', 'claude:attack', 'gemini:attack', 'codex:attack', 'claude:synthesize', 'codex:implement'];
    process.stderr.write(JSON.stringify({ type: 'council_mode', mode: 'adversarial' }) + '\n');
    await runAdversarialCouncil(prompt, report, { preview, timeoutMs, specContent, promptHash });
  } else {
