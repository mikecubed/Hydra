...
import { detectModelError, recoverFromModelError, detectCodexError } from './hydra-model-recovery.mjs';
  ...
  async _workLoop() {
    while (!this._stopped) {
      try {
        ...
        if (!result.ok) {
          // Check for Codex-specific errors first (auth/sandbox/invocation)
