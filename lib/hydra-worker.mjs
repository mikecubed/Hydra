...
function buildHeadlessCommand(agent, prompt, opts = {}) {
  ...
  switch (agent) {
    case 'claude':
      return {
        cmd: 'claude',
        args: ['-p', prompt, '--output-format', 'json', '--permission-mode', permissionMode],
      };
    case 'gemini':
      return {
        cmd: 'gemini',
        args: ['-p', prompt, '--approval-mode', permissionMode, '-o', 'json'],
      };
    case 'codex': {
...
export class AgentWorker extends EventEmitter {
  ...
  _executeAgent(prompt) {
    return new Promise((resolve) => {
      ...
      const { cmd, args } = buildHeadlessCommand(this.agent, prompt, {
