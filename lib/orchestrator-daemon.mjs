...
import { git, getCurrentBranch as getGitCurrentBranch, smartMerge } from './hydra-shared/git-ops.mjs';
...
function createTaskWorktree(taskId) {
  ...
  try {
    ...
    console.warn(`[worktree] Exception creating worktree for task ${taskId}: ${e.message}`);
    return null;
  }
}
...
function mergeTaskWorktree(taskId) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      console.warn(`[worktree] Conflict merging task ${taskId} branch into ${currentBranch}: ${conflictList}`);
      return { ok: false, conflict: true };
    }
    return { ok: true };
  } catch (e) {
    console.warn(`[worktree] Exception merging task ${taskId}: ${e.message}`);
...
function cleanupTaskWorktree(taskId, { force = false } = {}) {
  ...
  try {
    ...
    if (result.status !== 0) {
      console.warn(`[worktree] Could not remove worktree for task ${taskId}: ${(result.stderr || '').trim()}`);
    }
  } catch (e) {
    console.warn(`[worktree] Exception removing worktree for task ${taskId}: ${e.message}`);
  }
  ...
  try {
    ...
    console.warn(`[worktree] Exception deleting branch ${branch}: ${e.message}`);
  }
}
