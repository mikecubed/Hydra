...
function suggestNext(state, agent) {
  ...
  const inProgress = openTasks.find(
    (task) => task.owner === agent && task.status === 'in_progress',
  );
  if (inProgress) {
    return {
...
async function commandStatus(options) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Daemon status check failed (${response.status}): ${payload.error || 'unknown error'}`,
      );
      process.exit(1);
    }
    console.log(JSON.stringify(payload, null, 2));
  } catch (err) {
    console.error(`Daemon not reachable at ${url}: ${err.message}`);
    process.exit(1);
  }
}
...
async function commandStop(options) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Failed to stop daemon (${response.status}): ${payload.error || 'unknown error'}`,
      );
      process.exit(1);
    }
    console.log('Stop signal sent to orchestrator daemon.');
  } catch (err) {
    console.error(`Unable to reach daemon at ${url}: ${err.message}`);
    process.exit(1);
  }
}
 ...
 * @returns {string|null}
 */
function createTaskWorktree(taskId) {
  ...
  try {
    ...
    console.warn(`[worktree] Exception creating worktree for task ${taskId}: ${err.message}`);
    return null;
  }
}
 ...
 */
function mergeTaskWorktree(taskId) {
  const branch = `hydra/task/${taskId}`;
  ...
  try {
    ...
    console.warn(`[worktree] Exception merging task ${taskId}: ${err.message}`);
    return { ok: false, error: err.message };
  }
}
 ...
 */
function cleanupTaskWorktree(taskId, { force = false } = {}) {
  const cfg = loadHydraConfig();
  const worktreeDir = cfg.routing?.worktreeIsolation?.worktreeDir || '.hydra/worktrees';
  const worktreePath = path.resolve(config.projectRoot, worktreeDir, `task-${taskId}`);
  const branch = `hydra/task/${taskId}`;
  ...
  try {
    ...
    console.warn(`[worktree] Exception removing worktree for task ${taskId}: ${err.message}`);
  }
  ...
  try {
    ...
    console.warn(`[worktree] Exception deleting branch ${branch}: ${err.message}`);
  }
}
...
function startDaemon(options) {
  ...
  function runVerification(taskId, plan) {
    ...
    try {
      exec(
        plan.command,
        { cwd: config.projectRoot, timeout: plan.timeoutMs, encoding: 'utf8' },
