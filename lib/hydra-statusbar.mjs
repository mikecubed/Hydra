...
export function drawStatusBar() {
  ...
  // Re-establish scroll region every draw to recover from corruption
  // (child processes, escape sequences from agents, etc. can reset it)
  const scrollBottom = rows - STATUS_BAR_HEIGHT;
  process.stdout.write(`${ESC}1;${scrollBottom}r`);
...
export function initStatusBar(agents) {
  ...
  refreshInterval = setInterval(() => {
    if (statusBarActive) drawStatusBar();
  }, REFRESH_INTERVAL_MS);
