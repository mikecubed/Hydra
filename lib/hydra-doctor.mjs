...
/**
 * Classify an issue as transient (clear-only), config (needs config change), or code (needs agent fix).
 */
function classifyIssueType(entry) {
  const error = (entry.error || '').toLowerCase();
  const rootCause = (entry.rootCause || '').toLowerCase();

  // Transient: timeouts, segfaults, generic agent crashes, rate limits
  if (entry.timedOut) return 'transient';
