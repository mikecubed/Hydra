...
// ── Reasoning effort display per provider ────────────────────────────────────
// Each provider uses different terminology for their thinking/reasoning feature.
const EFFORT_LABELS = {
  openai:    { low: 'low',     medium: 'medium',  high: 'high' },
  anthropic: { low: 'concise', medium: 'balanced', high: 'verbose' },
  google:    { low: 'lite',    medium: 'standard', high: 'deep' },
};
function formatEffort(model, effort) {
  if (!effort) return '';
  const provider = /^claude/.test(model) ? 'anthropic'
    : /^gemini/.test(model) ? 'google' : 'openai';
  const mapped = EFFORT_LABELS[provider]?.[effort] || effort;
  return mapped;
}
...
async function printWelcome(baseUrl) {
  ...
  try {
    ...
    for (const [agent, info] of Object.entries(models)) {
      ...
      const eff = info.reasoningEffort ? pc.yellow(` ${formatEffort(info.active, info.reasoningEffort)}`) : '';
    ...
    try {
      ...
      if (line === ':model' || line.startsWith(':model ')) {
        ...
        if (modelArgs) {
          ...
          for (const [agent, info] of Object.entries(summary)) {
            ...
            const effort = info.reasoningEffort ? pc.yellow(` [${formatEffort(info.active, info.reasoningEffort)}]`) : '';
      ...
      if (line === ':roles') {
        ...
        for (const [role, rc] of Object.entries(roles)) {
          ...
          const effortStr = rc.reasoningEffort ? pc.yellow(` [${formatEffort(rc.model || '', rc.reasoningEffort)}]`) : '';
