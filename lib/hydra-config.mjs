...
function getPackagedRuntimeRoot() {
  switch (process.platform) {
    case 'win32':
      return path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'Hydra');
    case 'darwin':
    ...
    default: // linux and others — XDG_DATA_HOME when set, else ~/.local/share (no display/session needed)
      return path.join(process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local', 'share'), 'Hydra');
  }
}
