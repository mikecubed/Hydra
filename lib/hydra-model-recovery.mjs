...
import { getActiveModel, setActiveModel, getAgent } from './hydra-agents.mjs';
...
export async function verifyAgentQuota(agent, opts = {}) {
  ...
  try {
    ...
    const apiKeyEnvMap = { codex: 'OPENAI_API_KEY', claude: 'ANTHROPIC_API_KEY', gemini: 'GEMINI_API_KEY' };
    const apiKey = process.env[apiKeyEnvMap[agent]] || process.env.GOOGLE_API_KEY || null;
    if (agentDef?.quotaVerify) {
      return await agentDef.quotaVerify(apiKey, { hintText });
    }
