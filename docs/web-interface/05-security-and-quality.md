...
| **Gaps**              | Reconnect retry configuration (max attempts, backoff range) is not exposed as a tunable. Connection banner does not show reconnect attempt count or estimated wait. Browser-side reconnect is delegated to `useSession` WebSocket logic — no dedicated reconnect manager with circuit-breaker semantics. **T012** should surface reconnect progress; **T014** should expose tunable retry configuration; **T016** should add browser drill specs for the full reconnect → exhaustion → manual recovery arc. 🔧 |
