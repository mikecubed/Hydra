...
- **Deferred**: `rf-cs03` (usage tracking consolidation is no longer on the critical path)
- **Partial / follow-on remaining**:
  - `rf-ab06` / `rf-ab07` — typed exports landed, but consumer adoption through DI is still pending
  - `rf-pl02` — 5 of 29 direct `process.exit()` call sites still remain in `lib/`
- **Highest-priority remaining task**: `rf-ab01` (`IHydraConfig`)
...
| `rf-cs03` | Consolidate usage tracking           | Replace inline usage updates with a tested shared path                                                    | `rf-sn07`, `rf-cy03`                                  | `.worktrees/rf-cs03-usage-tracking`        | `gpt-5.4`           | `claude-sonnet-4.6`    | ⏸ Deferred                        | Usage bookkeeping has one tested entry point                                            |
...
| `rf-ab01` | Introduce `IHydraConfig`             | Define the interface and migrate covered consumers in batches                                             | `rf-cs02`, `rf-sn08`, `rf-sn09`, `rf-sn10`, `rf-sn11` | `.worktrees/rf-ab01-ihydraconfig`          | `claude-sonnet-4.6` | `gpt-5.4`              | ⬜ Not started                    | Interface is real, not aspirational, and each migrated consumer is test-backed          |
...
| `rf-ab06` | Introduce `IMetricsRecorder`         | Extract and migrate a metrics-recording seam after the metrics safety net exists                          | `rf-sn06`, `rf-ab05`                                  | `.worktrees/rf-ab06-imetricsrecorder`      | `claude-sonnet-4.6` | `gpt-5.4`              | ⚠ Partial — typed export landed   | Metrics recording is isolated behind a tested interface                                 |
| `rf-ab07` | Introduce `IGitOperations`           | Extract and migrate shared git operations after executor/evolve seams settle                              | `rf-ev02`, `rf-ab05`                                  | `.worktrees/rf-ab07-igitoperations`        | `claude-sonnet-4.6` | `gpt-5.4`              | ⚠ Partial — typed export landed   | Git operations are isolated behind a tested interface                                   |
...
| `rf-pl02` | Replace `process.exit()`             | Convert exit paths to testable control flow only in modules that already have adequate tests              | `rf-ab05`, `rf-sn08`, `rf-sn09`, `rf-sn10`, `rf-sn11` | `.worktrees/rf-pl02-process-exit`          | `gpt-5.4`           | `gemini-3-pro-preview` | ⚠ Partial — 5 direct calls remain | Every changed exit path is backed by tests and remains lint-clean                       |
