...
- **`hydra-process.ts`** — Injectable process exit handler for testability. **Phase 5**: replaces all direct `process.exit()` calls. Exports `exit(code?)` (the replacement for `process.exit()` — respects the installed handler), `setExitHandler(fn)` (installs a custom handler, e.g. in tests to capture exit codes instead of terminating), `resetExitHandler()` (restores the default `process.exit` behavior). 12 call sites migrated across 7 files (`hydra-audit.ts`, `hydra-mcp-server.ts`, `hydra-operator.ts`, `hydra-tasks.ts`, `hydra-usage.ts`, `orchestrator-daemon.ts`, `daemon/write-routes.ts`). 3 tests in `test/hydra-process.test.ts`.
