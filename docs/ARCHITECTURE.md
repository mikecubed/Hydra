...
- **`hydra-operator.ts`** — Interactive command center entry point. 5 orchestration modes (auto, council, dispatch, smart, chat). Auto mode uses 3-way routing: single (fast-path), tandem (`publishTandemDelegation()` creates 2 tasks + 2 handoffs), or council (direct, skipping mini-round triage). Council gate warns when council is overkill. Fully decomposed in Phase 3 — the monolith was split into focused sub-modules (see below); `hydra-operator.ts` is now the thin coordinator that wires them together.
  - **`hydra-operator-commands.ts`** — Operator command registry and handlers (`:status`, `:dispatch`, `:roster`, `:agents`, `:persona`, etc.).
  - **`hydra-operator-dispatch.ts`** — Agent dispatch coordination from the REPL. Calls `buildAgentContext()`, routes to daemon, and handles tandem/council escalation.
  - **`hydra-operator-session.ts`** — Session state management (active sessions, fork/spawn, pause/resume).
  - **`hydra-operator-workers.ts`** — Worker lifecycle management (start, stop, restart, health checks).
  - **`hydra-operator-startup.ts`** — Startup initialization (daemon ping, agent head launch, hook registration).
  - **`hydra-operator-concierge.ts`** — Concierge integration for the REPL (streaming responses, event push, provider switching).
  - **`hydra-operator-ui.ts`** — UI helpers for the REPL (box rendering, formatted output, confirmation prompts).
  - **`hydra-operator-ghost-text.ts`** — Ghost text / inline completion. After `:status` with blocked tasks shows a Tab-submittable suggestion (deterministic + async AI upgrade via `conciergeSuggest()`).
  - **`hydra-operator-self-awareness.ts`** — Self-awareness snapshot (codebase summary injected into operator context).
- **`orchestrator-daemon.mjs`** — HTTP server with event-sourced state. Routes split into `daemon/read-routes.mjs` and `daemon/write-routes.mjs`. State persistence split into `daemon/state.ts` (readState/writeState/appendEvent/replayEvents), `daemon/archive.ts` (readEvents/readArchive/archiveState/createSnapshot/cleanOldSnapshots/checkIdempotency), and `daemon/worktree.ts` (createTaskWorktree/mergeTaskWorktree/cleanupTaskWorktree). Handles task lifecycle, handoffs, sessions, worktrees. Heartbeat-based crash recovery: stale detector runs every 60s, requeues tasks with expired heartbeats (configurable timeout), moves exhausted-retry tasks to dead-letter queue. Dead-letter queue for exhausted-retry tasks (failCount >= maxAttempts). Event snapshots (`docs/coordination/snapshots/`) with auto-compaction. Idempotency keys (5min TTL) for mutation endpoints. Task-level worktree isolation: `createTaskWorktree(taskId)` called on `POST /task/claim` (when `routing.worktreeIsolation.enabled`), `mergeTaskWorktree(taskId)` and `cleanupTaskWorktree(taskId)` called on `POST /task/result`. Endpoints: `GET /dead-letter`, `POST /dead-letter/retry`, `POST /admin/compact`, `POST /task/:id/heartbeat`. Config: `workers.retry.maxAttempts`, `workers.heartbeatTimeoutMs`, `daemon.snapshot.everyNEvents`, `.retentionCount`.
...
- **`hydra-process.ts`** — Injectable process exit handler for testability. Introduced in **Phase 5** and being adopted incrementally. Exports `exit(code?)` (the replacement for `process.exit()` — respects the installed handler), `setExitHandler(fn)` (installs a custom handler, e.g. in tests to capture exit codes instead of terminating), `resetExitHandler()` (restores the default `process.exit` behavior). 29 `exit()` call sites migrated across 10 files (`hydra-actualize-review.ts`, `hydra-actualize.ts`, `hydra-audit.ts`, `hydra-eval.ts`, `hydra-evolve-review.ts`, `hydra-mcp-server.ts`, `hydra-nightly-review.ts`, `hydra-operator.ts`, `hydra-tasks.ts`, `hydra-usage.ts`, `orchestrator-daemon.ts`, `daemon/write-routes.ts`, `sync.ts`); ~5 direct `process.exit()` calls remain in non-interactive paths. 3 tests in `test/hydra-process.test.ts`.
...
- **`lib/types.ts`** — Central shared type definitions. **Phase 5**: added formal interface contracts to improve testability and decouple consumers. Exports: `IContextProvider` (contract for context assembly — `buildAgentContext(agent, opts)`), `IGitOperations` (contract for git command abstraction — `run(args)`, `getCurrentBranch()`, `checkoutBranch()`, `createBranch()`), `IMetricsRecorder` (contract for metrics recording — `recordCallStart()`, `recordCallComplete()`), `GitResult` (return type for `IGitOperations.run()`), `MetricsCallResult` (return type for `IMetricsRecorder.recordCallComplete()`). 8 contract tests in `test/hydra-interfaces.test.ts`.
...
Layer boundaries are enforced by `eslint-plugin-boundaries` (configured in `eslint.config.mjs`) and run in CI on every PR. Violations are `error` severity — 0 violations are currently allowed to merge.

| Layer     | Pattern               | May import from           |
| --------- | --------------------- | ------------------------- |
| `shared`  | `lib/hydra-shared/**` | `shared`, `lib`           |
| `daemon`  | `lib/daemon/**`       | `shared`, `daemon`, `lib` |
| `lib`     | `lib/**`              | `shared`, `daemon`, `lib` |
| `bin`     | `bin/**`              | `lib`, `shared`, `daemon` |
| `scripts` | `scripts/**`          | `lib`, `shared`           |
| `test`    | `test/**`             | all layers                |

The key invariant is that `shared` never imports from `lib` peers (only upward or within itself), preventing circular dependencies between the shared utilities and the feature modules that consume them.
