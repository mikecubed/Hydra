...
- **`hydra-process.ts`** — Injectable process exit handler for testability. Introduced in **Phase 5** and being adopted incrementally. Exports `exit(code?)` (the replacement for `process.exit()` — respects the installed handler), `setExitHandler(fn)` (installs a custom handler, e.g. in tests to capture exit codes instead of terminating), `resetExitHandler()` (restores the default `process.exit` behavior). 12 call sites migrated across 7 files so far (`hydra-audit.ts`, `hydra-mcp-server.ts`, `hydra-operator.ts`, `hydra-tasks.ts`, `hydra-usage.ts`, `orchestrator-daemon.ts`, `daemon/write-routes.ts`); ~9 files still use direct `process.exit()` and are being migrated in a parallel remediation track. 3 tests in `test/hydra-process.test.ts`.
