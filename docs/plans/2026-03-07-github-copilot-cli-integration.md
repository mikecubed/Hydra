...
| **Auth requirement**      | GitHub account with active Copilot subscription; **browser-based device-flow auth only** — run `copilot` interactively once to authenticate. No env var or headless auth path. Hydra treats Copilot as optimistically authenticated; auth failures surface at task-execution time.                       |
...
copilot: {
  ...
  errorPatterns: {
    // authRequired match → skip Copilot for the remainder of the session only.
    // Other error types (rateLimited, networkError, quotaExhausted) must NOT
    // trigger a session skip — they are transient or quota-related, not auth failures.
  ...
  // no env var or status command to verify. Assume authenticated; if a task fails
  // with an authRequired error pattern, the executor skips Copilot for the session.
  ...
  weaknesses: [
    ...
    'github-account-auth',     // Browser-only device-flow auth; must authenticate interactively before first use
...
5. Include instructions for authentication — run `copilot` interactively once to complete browser-based device-flow login before first use with Hydra. No env var or headless auth path is available.
...
| Auth flow in CI/headless                | **High**   | **Browser-based device-flow auth only — no env var or headless auth path exists.** Hydra uses an optimistic strategy: assume Copilot is authenticated; skip it for the remainder of the session only when a task fails with an `authRequired` error pattern match. Other error types (rate limit, network, quota) must not trigger a session skip. CI pipelines will effectively never use Copilot. Document clearly: run `copilot` interactively once before starting Hydra.                                                                                                 |
...
| Copilot installed but not authenticated | **Medium** | `detectInstalledCLIs()` checks binary existence only. There is no `copilot auth status` command and no reliable token file to probe — auth state is only knowable from a live run. Hydra uses an **optimistic model**: Copilot is assumed authenticated. If a task fails and stderr/stdout matches `errorPatterns.authRequired`, the executor marks Copilot as skipped for the session and surfaces: `"Copilot is not authenticated — run 'copilot' interactively to log in"`. Non-auth errors (rate limit, network, quota, tool failure) must not set the session-skip flag. |
